...
output: 
  flexdashboard::flex_dashboard:
    orientation: rows
    social: menu
    source_code: https://github.com/mine-cetinkaya-rundel/rladies-phl-shiny/blob/master/01-flexdash/un-women-dash/un-women-dash.Rmd
runtime: shiny
---
...
unwomen <- read_csv("../../data/un-women/un-women.csv") %>% 
  dplyr::filter(
    !is.na(region),
    sex == "Female"
    ) %>%
...
selectInput(
  ...
  choices = c(
    ...
    "Average number of hours spent on paid and unpaid domestic and care work combined"
      = "hrs_dom_care_work",
    "Labour force participation rate for persons aged 15-24" = "labor_force",
...
xlab <- reactive({
  case_when(
    ...
    input$x == "hrs_dom_care_work"        ~  
      "Average number of hours spent on paid and unpaid domestic and care work combined",
    input$x == "labor_force"              ~ 
      "Labour force participation rate for persons aged 15-24",
